  ...
  return <div className="flex h-full flex-col overflow-hidden">
    ...
    <div className="grid flex-1 gap-6 overflow-y-auto p-6 lg:grid-cols-[minmax(0,1fr)_360px]">
      <Card><CardHeader><CardTitle className="text-sm">Importar archivo</CardTitle></CardHeader><CardContent className="space-y-4">
        ...
        <div className="flex flex-wrap gap-2"><Button variant="outline" onClick={validatePreview} disabled={!file || !modelId || uploading}>Validar previsualización</Button><Button onClick={submit} disabled={!file || !modelId || uploading} className="gap-2">{uploading ? <Loader2 className="h-4 w-4 animate-spin" /> : <FileUp className="h-4 w-4" />} Importar y validar</Button></div>
      </CardContent></Card>
