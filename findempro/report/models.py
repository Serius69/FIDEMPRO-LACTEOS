...
class Report(models.Model):
    ...
    @classmethod
    def create_from_simulation(cls, product, simulation_data, title=None):
        """Método de clase para crear reporte desde datos de simulación."""
        title = title or f"Simulación {product.name} - {timezone.now().strftime('%Y-%m-%d')}"
        content = simulation_data if isinstance(simulation_data, dict) else {}
