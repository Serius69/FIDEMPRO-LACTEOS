...
class DashboardService:
    ...
    def _get_enhanced_performance_trends(cls, business_id: int) -> Dict[str, Any]:
        """Obtiene tendencias de rendimiento mejoradas"""
        try:
            today = timezone.now()
            ...
            for i in range(6):
                ...
                month_simulations = Simulation.objects.filter(
                    fk_questionary_result__fk_questionary__fk_product__fk_business=business_id,
                ...
                month_metrics = cls._calculate_enhanced_metrics(list(month_simulations))
                ...
                monthly_data.append({
                    ...
                    'simulations_count': month_simulations.count()
                })
    ...
    def _get_enhanced_top_products(cls, business_id: int) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for product in products:
                # Calcular métricas adicionales por producto
                product_simulations = Simulation.objects.filter(
                    fk_questionary_result__fk_questionary__fk_product=product
                )[:10]  # Últimas 10 simulaciones
                
                product_metrics = cls._calculate_enhanced_metrics(list(product_simulations))
