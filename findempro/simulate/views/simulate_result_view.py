...
class SimulateResultView(LoginRequiredMixin, View):
    ...
    def _prepare_complete_results_context(self, simulation_id, simulation_instance, results_simulation, historical_demand):
        ...
        try:
            ...
            # ── FASE 3.2: Risk metrics for percentile table ───────────────
            try:
                profit_samples = np.array([
                    float(day.get('GT', 0))
                    for day in all_variables_extracted
                    if day.get('GT') is not None
                ], dtype=float)
                if profit_samples.size >= 4:
                    pct = lambda q: float(np.percentile(profit_samples, q))
                    mean_p = float(np.mean(profit_samples))
                    std_p  = float(np.std(profit_samples))
                    var_95  = pct(5)
