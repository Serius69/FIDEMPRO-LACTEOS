...
# Canvas v2 models — re-exported so Django migration system discovers them
from .canvas_models import (  # noqa: F401
    SimulationProject,
    ModelNode,
    ModelEdge,
    InterfacePage,
...
@receiver(post_save, sender=Business)
def update_business_distributions(sender, instance, created, **kwargs):
    """Sincroniza is_active de PDFs cuando el Business se actualiza (no en creación)."""
    if created:
        return  # create_probabilistic_density_functions ya manejó la creación
...
class SimulationShareToken(models.Model):
    """Temporary public share link for a simulation result (expires in 7 days by default)."""

    token       = models.UUIDField(default=uuid.uuid4, unique=True, editable=False, db_index=True)
    simulation  = models.ForeignKey(
        Simulation, on_delete=models.CASCADE, related_name='share_tokens',
    )
    created_by  = models.ForeignKey(
        'auth.User', null=True, on_delete=models.SET_NULL, related_name='simulation_share_tokens',
    )
    ...
    class Meta:
        ordering = ['-created_at']
        indexes  = [models.Index(fields=['token', 'is_active'], name='share_token_lookup')]

    def __str__(self):
        return f'Share {self.token} → simulation {self.simulation_id}'
    ...
    @classmethod
    def create_for(cls, simulation: 'Simulation', user, days: int = 7) -> 'SimulationShareToken':
        from django.utils import timezone
...
class RiskAlert(models.Model):
    """
    Alerta de riesgo VaR/CVaR configurable por usuario.

    Después de cada simulación se evalúa si el VaR cae por debajo
    del umbral definido; si se cumple la condición, se envía un email.
    Compatible con v1 (Simulation) y v2 canvas (SimulationProject).
    """
    ...
    fk_product = models.ForeignKey(
        ...
        blank=True,
        related_name='risk_alerts',
        help_text='Producto asociado (v1). Dejar en blanco para proyectos canvas v2.',
    )
    var_threshold = models.FloatField(
        help_text='Umbral de VaR (BOB). Alerta si VaR calculado < este valor.',
    )
    cvar_threshold = models.FloatField(
        null=True,
        blank=True,
        help_text='Umbral opcional de CVaR/Expected Shortfall (BOB).',
    )
    email = models.EmailField(
        help_text='Dirección de correo que recibirá la notificación.',
    )
    is_active = models.BooleanField(default=True)
    created_at = models.DateTimeField(auto_now_add=True)
    ...
    def is_triggered(self, var_value: float, cvar_value: float | None = None) -> bool:
        ...
        if self.var_threshold is not None and var_value < self.var_threshold:
            return True
        if (
            self.cvar_threshold is not None
            and cvar_value is not None
            and cvar_value < self.cvar_threshold
        ):
            return True
        return False