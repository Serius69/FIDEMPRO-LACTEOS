...
class SimulationCore:
    ...
    def _build_initial_state(self, simulation_instance) -> dict:
        """
        Construye el estado inicial de la simulación desde el cuestionario.
        Usa variables de tipo Estado (type=2) del producto, con fallback a CompanyProfile.
        Genérico: no asume ningún sector específico.
        """
        from variable.models import Variable

        product = simulation_instance.fk_questionary_result.fk_questionary.fk_product
        state_vars = Variable.objects.filter(
            fk_product=product, type=2, is_active=True
        ).values('initials')
        state_var_names = {v['initials'] for v in state_vars}

        # Buscar valores en las respuestas del cuestionario
        extracted = self.variable_mapper.extract_all_variables(
            simulation_instance.fk_questionary_result
        )

        inventories = {k: v for k, v in extracted.items() if k in state_var_names}

        # Si CompanyProfile tiene un initial_state definido, usarlo como fallback
        try:
            from business.models import CompanyProfile
            profile = CompanyProfile.objects.get(
                fk_business=product.fk_business
            )
            if hasattr(profile, 'custom_kpis') and 'initial_state' in profile.custom_kpis:
                for k, v in profile.custom_kpis['initial_state'].items():
                    inventories.setdefault(k, float(v))
        except Exception:
            pass

        logger.info(
            f"Estado inicial cargado: {len(inventories)} variables de estado "
            f"({list(inventories.keys())})"
        )
    ...
    def _update_simulation_state(self, state, day_results):
        """
        Actualiza el estado de la simulación con los resultados del día.
        Genérico: persiste cualquier variable que ya estaba en state['inventories'].
        """
        results = day_results['endogenous_results']

        # Actualizar todas las variables de estado que se calcularon este día
        for key in list(state['inventories'].keys()):
            if key in results:
                state['inventories'][key] = results[key]

        # Guardar históricos usando la variable de demanda predicha
        if 'predicted_demand' in day_results:
            state['previous_demands'].append(day_results['predicted_demand'])

        # Guardar ventas y producción usando nombres canónicos si existen
        for sales_var in ('TPV', 'VENTAS', 'INGRESOS_UNIDADES'):
            if sales_var in results:
                state['previous_sales'].append(results[sales_var])
                break

        for prod_var in ('QPL', 'PRODUCCION', 'CANTIDAD_PRODUCIDA'):
            if prod_var in results:
                state['previous_production'].append(results[prod_var])
                break

        # Mantener solo los últimos 30 períodos de histórico
    ...
    def _prepare_simulation_data(self, simulation_instance):
        ...
        logger.info(f"Loaded {len(answers_list)} answers from questionnaire")
    ...
    def _simulate_single_day_complete(self, simulation_instance, simulation_data, day_index, simulation_state):
        ...
        if day_index > 0:
            # Inyectar todas las variables de estado del período anterior
            for k, v in simulation_state['inventories'].items():
                variable_dict[k] = v
    ...
    def _add_minimal_defaults(self, variable_dict):
        """
        Agrega solo defaults universales (no específicos de ningún sector).
        Los defaults de sector deben venir del cuestionario o de CompanyProfile.
        """
        # Defaults verdaderamente universales (operativos para cualquier empresa)
        universal_defaults = {
            'ED': 1.0,    # Factor de estacionalidad neutro
            'MLP': 480,   # Minutos laborables por día (8h estándar)
            'PI': 0.0,    # Pedidos en proceso inicial
            'UII': 0.0,   # Uso de inventario insumos inicial
        }

        # CPROD: si hay valor de producción, calcular capacidad como 20% sobre ese valor
        if 'QPL' in variable_dict and 'CPROD' not in variable_dict:
            universal_defaults['CPROD'] = variable_dict['QPL'] * 1.2

        added_defaults = []
        for var, default_val in universal_defaults.items():
            if var not in variable_dict:
                variable_dict[var] = default_val
                added_defaults.append(var)

        if added_defaults:
            logger.debug(f"Added universal defaults for: {added_defaults}")
    ...
    def _calculate_all_equations(self, equations, variable_dict):
        ...
        # Usar únicamente las ecuaciones de la base de datos (no datos hardcodeados de test)
