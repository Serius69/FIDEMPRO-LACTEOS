...
class DemandModelService:
    ...
    def analyze_trend(self) -> Dict:
        ...
        if abs(slope) < 0.01 * np.mean(self.data) or r_value ** 2 < 0.1:
            direction = 'stable'
        elif slope > 0:
    ...
    def _calculate_mape(self, method: str, trend: Dict) -> float:
        """Calcula MAPE usando validación cruzada simple (último 20% de datos)."""
        ...
        try:
            if method == 'linear':
                ...
                preds = np.array([intercept + slope * (len(train) + i) for i in range(n_test)])
            else:
    ...
    def to_simulation_params(self) -> Dict:
        """
        Convierte el análisis en parámetros listos para usar en SimulationConfig.
        Integración directa con MonteCarloEngine.
        """
        desc = self.descriptive_stats()
        best_dist, _, _, _ = self.fit_distribution()
        trend = self.analyze_trend()

        return {
            'distribution_type': best_dist,
            'demand_mean': desc['mean'],
            'demand_std': desc['std'],
            'demand_min': desc['min'],
            'demand_max': desc['max'],
            'trend_slope': trend['slope'],
