...
class Product(models.Model):
    ...
    def areas_count(self):
        ...
        return self.fk_product_area.filter(is_active=True).count()  # type: ignore

    @property
    def variables_count(self):
        """Contar variables asociadas al producto"""
        return self.fk_product_variable.filter(is_active=True).count()  # type: ignore
...
class Area(models.Model):
    ...
    def equations_count(self):
        """Contar ecuaciones activas asociadas al área"""
        return self.area_equation.filter(is_active=True).count()  # type: ignore
...
def optimize_image(sender, instance, **kwargs):
    ...
    if not instance.image_src:
        return
    # Skip processing if the image field hasn't changed
    if instance.pk:
        try:
            old = sender.objects.only('image_src').get(pk=instance.pk)
            if old.image_src.name == instance.image_src.name:
                return
...
def update_product_ready_status(sender, instance, **kwargs):
    ...
    if instance.is_active:
        areas_count = instance.fk_product_area.filter(is_active=True).count()  # type: ignore
