...
# La vista de detalles usa @cache_page; limpiamos el caché entre tests para que
# no sirva una respuesta previa (los pks pueden reutilizarse al hacer rollback).
...
# Cabecera para forzar la rama AJAX de las vistas (que devuelve JSON + status codes)
...
def business(user):
    return Business.objects.create(
        ...
        description="Descripcion de prueba del negocio",
        is_active=True,
    )
...
def test_read_business_view(client, business, product):
    url = reverse("business:business.overview", args=[business.id])
    response = client.get(url)
    assert response.status_code == 200
...
def test_create_business_view(client):
    url = reverse("business:business.create")
    data = {
        "name": "New Business",
        "type": Business.BusinessType.RETAIL,
        "location": "New Location",
        "description": "Descripcion valida del nuevo negocio",
    }
    response = client.post(url, data, **AJAX)
    assert response.status_code == 200
    assert Business.objects.filter(name="New Business").exists()

def test_update_business_view(client, business):
    url = reverse("business:business.update", args=[business.id])
    data = {
        "name": "Updated Business",
        "type": Business.BusinessType.RETAIL,
        "location": "Updated Location",
        "description": "Descripcion valida del negocio actualizado",
    }
    response = client.post(url, data, **AJAX)
    assert response.status_code == 200
...
def test_get_business_details_view(client, business):
    url = reverse("business:business.get_business_details_view", args=[business.id])
    response = client.get(url)
    assert response.status_code == 200
    data = response.json()
...
def test_get_business_details_view_not_found(client):
    url = reverse("business:business.get_business_details_view", args=[999])
    response = client.get(url)
...
def test_business_list_view_pagination(client, user):
    # Create multiple businesses to test pagination (12 por página)
    for i in range(15):
...
def test_read_business_view_pagination(client, business):
    # Create multiple products to test pagination (10 por página)
    for i in range(15):
...
def test_delete_business_view_invalid_method(client, business):
    url = reverse("business:business.delete", args=[business.id])
    response = client.get(url, **AJAX)  # Invalid method (GET instead of POST)
