...
class FinanceRecommendation(models.Model):
    ...
    class Meta:
        ...
        ordering = ['-date_created']
        indexes = [
            models.Index(fields=['fk_business', 'is_active']),
...
def create_finance_recommendation(sender, instance, created, **kwargs):
    ...
    if not created:
        return
    FinanceRecommendation.objects.bulk_create([
...
@receiver(post_save, sender=Business)
def sync_finance_recommendation_status(sender, instance, **kwargs):
    """Sincroniza el estado activo de las recomendaciones con el del negocio."""
    update_fields = kwargs.get('update_fields')
    if update_fields is not None and 'is_active' not in update_fields:
        return
    instance.fk_business_finance_recommendation.update(is_active=instance.is_active)
