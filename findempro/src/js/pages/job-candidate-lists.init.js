...
function loadCandidateListData(e, t) {
  ...
  for (
    var n, s, i = (t - 1) * itemsPerPage;
    i < t * itemsPerPage && i < e.length;
    i++
  )
    e[i] &&
      ((n = e[i].bookmark ? "active" : ""),
      (s = e[i].userImg
        ? '<img src="' +
          e[i].userImg +
        ...
        : '<div class="avatar-title border bg-light text-primary rounded text-uppercase fs-16">' +
          ...
          "</div>"),
      (document.querySelector("#candidate-list").innerHTML +=
        '<div class="col-md-6 col-lg-12">        <div class="card mb-0">            <div class="card-body">                <div class="d-lg-flex align-items-center">                    <div class="flex-shrink-0">                        <div class="avatar-sm rounded">' +
        s +
        '</div>                    </div>                    <div class="ms-lg-3 my-3 my-lg-0">                        <a href="profile.html"><h5 class="fs-16 mb-2">' +
        e[i].candidateName +
        '</h5></a>                        <p class="text-muted mb-0">' +
        e[i].designation +
        '</p>                    </div>                    <div class="d-flex gap-4 mt-0 text-muted mx-auto">                        <div><i class="ri-map-pin-2-line text-primary me-1 align-bottom"></i> ' +
        e[i].location +
        '</div>                        <div><i class="ri-time-line text-primary me-1 align-bottom"></i> ' +
        isStatus(e[i].type) +
        '</div>                    </div>                    <div class="d-flex flex-wrap gap-2 align-items-center mx-auto my-3 my-lg-0">                        <div class="badge text-bg-success">                            <i class="mdi mdi-star me-1"></i>' +
        e[i].rating[0] +
        '                        </div>                        <div class="text-muted">' +
        e[i].rating[1] +
        '</div>                    </div>                    <div>                        <a href="#!" class="btn btn-soft-success">View Details</a>                        <a href="#!" class="btn btn-ghost-danger btn-icon custom-toggle ' +
        n +
        '" data-bs-toggle="button">                            <span class="icon-on"><i class="ri-bookmark-line align-bottom"></i></span>                            <span class="icon-off"><i class="ri-bookmark-3-fill align-bottom"></i></span>                        </a>                    </div>                </div>            </div>        </div>    </div>'));
  selectedPage(),
    1 == currentPage
...
getJSON("job-candidate-list.json", function (e, t) {
  null !== e
    ...
    : (loadCandidateListData((allcandidateList = t), currentPage),
      paginationEvents());
});
...
searchElementList.addEventListener("keyup", function () {
  ...
  for (
    var t,
      e = allcandidateList.filter(function (e) {
        return (
          -1 !== e.designation.toLowerCase().indexOf(t.toLowerCase()) ||
          -1 !== e.candidateName.toLowerCase().indexOf(t.toLowerCase())
        );
      }),
      a =
        (0 == e.length
